...
const SwapBox: React.FC<{}> = () => {
  ...
  const [tokenName1, setTokenName1] = useState("DAI");
  ...
  useEffect(() => {
    if (tokenServerData) {
      const token1 = TOKENS.find(({ name }) => name === tokenName1);
      if (token1 && token1.id in tokenServerData) {
        setUsd1(tokenServerData[token1.id].usd);
      }
      const token2 = TOKENS.find(({ name }) => name === tokenName2);
  ...
  // 토큰 가격 입력 후, 해당하는 usd 값이 변할 때마다 상대 토큰의 usd를 같은 값으로 변경
  useEffect(() => {
